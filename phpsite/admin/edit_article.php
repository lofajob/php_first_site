 			...
 			<?php 
				...
				if (!$db) {
					...
					if (!isset($_GET['id'])){											//если не нажали на ссылку, выводим список ссылок
						...
						echo ("<p class='p2'>Выберите статью для редактирования:</p>");					
						while ($myrow = mysql_fetch_array($result)){					//вывод ссылок
							echo("<p class='article_name'>
									<a href='edit_article.php?id=".$myrow['id']."'>".$myrow['title']."</a>
								  </p>");
						}
					}
					else{	
						...
						print("
							<form action='article_update.php' method='post' name='edit_article' class='form1'>
								...
								<input name='submit' type='submit' class='title' id='submit' value='Редактировать статью'>
								<br>                      
							</form> ");
					}
				}
